...
set -e
YOSYS="${YOSYS:-$(python -c "import sysconfig, pathlib; print(pathlib.Path(sysconfig.get_path('scripts')) / 'yowasp-yosys.exe'))"}"
if ! command -v "$YOSYS" >/dev/null 2>&1; then YOSYS="yowasp-yosys"; fi
