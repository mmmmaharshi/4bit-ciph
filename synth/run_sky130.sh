...
"$YOSYS" -p "read_verilog quartet_logic.v; hierarchy -check -top quartet_round_logic; proc; opt; techmap; opt; stat" 2>/dev/null || "$YOSYS.exe" -p "read_verilog quartet_logic.v; hierarchy -check -top quartet_round_logic; proc; opt; techmap; opt; stat" 2>/dev/null || yowasp-yosys -p "read_verilog quartet_logic.v; hierarchy -check -top quartet_round_logic; proc; opt; techmap; opt; stat" 2>/dev/null || yowasp-yosys.exe -p "read_verilog quartet_logic.v; hierarchy -check -top quartet_round_logic; proc; opt; techmap; opt; stat"
echo ""
