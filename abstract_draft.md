...
**(3) Generalization Across Designs.** We apply the method to 13 S-boxes from widely-used SPN ciphers. Twelve satisfy the Fourier vanishing condition (PRESENT, GIFT, PRINCE, Piccolo, TWINE, LED, SKINNY, Rectangle, LBlock, Serpent, HIGHT, AES); one (Camellia-s1) does not, identifying the structural boundary of applicability. S-boxes derived from field inversion over GF(2ᵐ) consistently satisfy the condition; composite-field constructions may not.
