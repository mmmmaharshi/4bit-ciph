...
3. Run the full test suite after any change to the cipher, the S-box, the key schedule, the test vectors, or the C/Python harness contract:
   ...
   ```
   All six must pass. The KAT is regenerated by `python tests/generate_kat.py`; if a cipher change alters the KAT, the diff is the expected impact and must be reviewed. The KAT regeneration should be a byte-identical no-op when the cipher is unchanged. The TVLA (`tests/tvla.py`) takes ~2 minutes; run it before any commit that touches the cipher core, the S-box tables, or the runner C code.
